...
View.prototype.exercise3 = function () {
  ...
  //your code here!
  $('div#easel').append("<h1>i love jquery</h1>");
};
...
View.prototype.exercise4 = function () {
  ...
  //your code here!
  const $allSquares = $('li.square');
  for (let i = 0; i < $allSquares.length; i++) {
    if (i % 2 !== 0) {
      let $liElement = $($allSquares[i]);
      $liElement.text('nandini');
    }
  }
};
...
View.prototype.exercise6 = function () {
  ...
  for (let i = 0; i < $listItems.length; i++) {
    let $liElement = $($listItems[i]);
    $liElement.attr("style", "background-color: " + window._randomColorString());
  }
};
...
View.prototype.exercise7 = function(){
  ...
  $listItems.on("mouseover", (event) => {
    const currentTarget = event.currentTarget;
    const $currentTarget = $(currentTarget);
    console.log($currentTarget.attr("style"));
  });
};
